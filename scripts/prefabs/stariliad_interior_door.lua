...
local function OnDoneTeleporting(inst, obj)
    local offset = out_offset[inst.direction] or Vector3(0, 0, 0)

    target.Transform:SetPosition((offset + inst:GetPosition()):Get())
    target:ForceFacePoint((target:GetPosition() + offset):Get())
end

local function MakeDoor(prefabname, data)
    local function fn()
        ...
        inst.direction = data.direction

        inst.AnimState:SetBank("player_house_doors")
        inst.AnimState:SetBuild("player_house_doors")
        inst.AnimState:PlayAnimation(inst.material .. "_door_open_" .. inst.direction)

        inst:AddTag("stariliad_interior_door")
...
local rets = {}
for prefabname, data in pairs(defines) do
    table.insert(rets, MakeDoor(prefabname, data))
end
