local assets =
{
    Asset("ANIM", "anim/stariliad_necrons_stele.zip"),
}

local MAX_WORKLEFT = 10

SetSharedLootTable("stariliad_necrons_stele",
    {
        { "marble", 1.00 },
        { "marble", 1.00 },
        { "marble", 0.33 },
...
local function CheckAnimAndLight(inst)
    if inst.anim_index == nil then
        inst.anim_index = math.random(1, 2)
    end
...
local function common_fn(anim_index)
    ...
    inst.AnimState:SetBank("stariliad_necrons_stele")
    inst.AnimState:SetBuild("stariliad_necrons_stele")

    local s = 0.5
    inst.AnimState:SetScale(s, s, s)

    -- for i = 1, 2 do
    --     inst.AnimState:SetSymbolLightOverride("type" .. i .. "_text", 1)
    -- end
    ...
    inst:SetPrefabName("stariliad_necrons_stele")
    ...
    inst.components.lootdropper:SetChanceLootTable("stariliad_necrons_stele")
...
return Prefab("stariliad_necrons_stele", fn, assets),
    Prefab("stariliad_necrons_stele1", fn_1, assets),
    Prefab("stariliad_necrons_stele2", fn_2, assets)
