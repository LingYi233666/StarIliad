...
local StariliadNecronsScarabBrain = Class(Brain, function(self, inst)
    Brain._ctor(self, inst)
end)

function StariliadNecronsScarabBrain:OnStart()
                ...
                PriorityNode(
                    {
                        ...
                        WhileNode(
                            ...
                            --     STOP_RUN_AWAY_DIST)
                            RunAway(self.inst, { getfn = function(inst) return inst.components.combat.target end },
                        ...
                        WhileNode(function()
                                return
                                    self.inst.components.combat.target == nil and self.inst:GetTimeAlive() > 5
                            end, "NoTarget",
                            DoAction(self.inst, GoHomeAction, "go home", true)),

                        WhileNode(
                            function() return GetHome(self.inst) end,
                            "HasHome",
...
function StariliadNecronsScarabBrain:OnInitializationComplete()
    if not self.inst.components.knownlocations:GetLocation("home") then
        self.inst.components.knownlocations:RememberLocation("home", Point(self.inst.Transform:GetWorldPosition()), true)
    end
end

return StariliadNecronsScarabBrain
